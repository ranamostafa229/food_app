...
const FillRecipesHeader = ({ action, title }) => {
  const { pathname } = useLocation();
  return (
    <header className="recipe-header d-flex justify-content-between  p-3 p-md-5">
      <div className="d-flex flex-column">
        <h4>
          {action} the <span className="text-success ">Recipes !</span>{" "}
        </h4>
        <span>
      ...
      <Link
        to={pathname === "/dashboard" ? "/recipes" : "/recipes"}
...
export default FillRecipesHeader;
